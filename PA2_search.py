...
def UCSearch(start, goal, barrier):
    ...
    while len(fringe) > 0:
        ...
        if curNode != goal:
            ...
            neighbours = get_neighbors(curNode)
            #check each neighbor of the current node using get_neighbors() function
            for el in neighbours:
                #path cost
                cost = move_cost(el, barrier)
                t_cost = cost + curGscore
                #if el in fringe and if new cost is lower update
                if el in fringe:
                    if G[el] > t_cost:
                        G[el] = t_cost
                        parent[el] = curNode

                #if el not in fringe check if it is in expandedNodes
                #if so, check if the new cost is lower and update
                if el not in fringe:
                    if el not in expandedNodes:
                     fringe.add(el)
                     G[el] = t_cost
                     parent[el] = curNode
                     if el in expandedNodes:
                         if G[el] > t_cost:
                            fringe.add(el)
                            G[el] = t_cost
                            parent[el] = curNode
    ...
    while len(fringe) > 0:
        ...
        if curNode != goal:
            ...
            for el in neighbours:
                cost = move_cost(el, barrier)
                h = heuristic(el, goal)
                total = G[curNode] + h + cost
                # if el in fringe and if new total cost is lower update
                if el in fringe:
                    if F[el] > total:
                        G[el] = G[curNode] + cost
                        F[el] = total
                ...
                # if el not in fringe check if it is in expandedNodes
                # if so, check if the new total cost is lower and update
                if el not in fringe:
                    if el not in expandedNodes:
                        fringe.add(el)
                        G[el] = G[curNode] + cost
